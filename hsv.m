...
for i = 1:numel(imageFiles)
    ...
    % Step 2: Extract the H, S, and V channels
    hChannel = hsvImg(:, :, 1);
    sChannel = hsvImg(:, :, 2);
    vChannel = hsvImg(:, :, 3);

    % Step 3: Skin color segmentation based on H and S channels
    skinMask = (hChannel > 0.01 & hChannel < 0.1) & (sChannel > 0.2 & sChannel < 0.7);
